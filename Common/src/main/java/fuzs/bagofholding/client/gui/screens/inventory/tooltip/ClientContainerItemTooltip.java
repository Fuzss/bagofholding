...
public class ClientContainerItemTooltip implements ClientTooltipComponent {
   ...
   private boolean hideInventoryContents() {
      return BagOfHolding.CONFIG.get(ClientConfig.class).contentsRequireShift && !Proxy.INSTANCE.hasShiftDown();
   }

   private int getLastFilledSlot() {
      if (BagOfHolding.CONFIG.get(ClientConfig.class).slotOverlay) {
   ...
   private float[] getBackgroundColor() {
      if (!BagOfHolding.CONFIG.get(ClientConfig.class).colorfulTooltips || this.backgroundColor == null) {
